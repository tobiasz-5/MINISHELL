...
void create_real_token(t_token_state *state, t_token_node **head, t_token_node **tail)
{
    t_token_node *new_node;

    new_node = create_token_node(state->token_str); //crea effettivamente il nodo
    if (new_node)   //se il nodo e' stato creato correttamente
    {
        new_node->single_quote = state->single_quote;   //imposta i campi della struttura t_token_node
...
/* Le due funzioni: process_quoted_token e process_normal_token lavorano insieme per creare un token.
...
int process_quoted_token(char *input, int i, char quote, t_token_state *state)
{
    ...
    start = i;
    end = find_closing_quote(input, i, quote); //trova la posizione della virgoletta di chiusura, se non la trova ritorna -1
    if (end == -1)
    {
        handle_unclosed_quote_error(state);
        return -1;
    }
    temp_token = strndup(input + start, end - start); //estrae il contenuto tra le virgolette
    if (!temp_token)
        return -1;
    result = append_to_temp_token_str(state, temp_token); //aggiunge il contenuto al token corrente
    free(temp_token);
    if (result == -1)
        return -1;
    set_quote_flag(state, quote);
    i = end + 1;         //aggiorna l'indice per continuare dopo la virgoletta di chiusura
...
{
    int start;
    char *temp_token;   //variabile temporanea per memorizzare il token
    char *new_str;      
    ...
    if (!temp_token)
        return -1;
    new_str = ft_strjoin(state->token_str, temp_token); //concatena il nuovo segmento di token al token corrente
    free(state->token_str);
    free(temp_token);
    if (!new_str)
        return -1;
...
int process_token(char *input, int i, t_token_state *state)
{
    ...
    if (input[i] == '\'' || input[i] == '"') //se il carattere corrente è una virgoletta, chiama process_quoted_token, altrimenti, process_normal_token.
    {
        quote = input[i];
...
int process_next_token(char *input, int i, t_token_state *state, t_token_node **head, t_token_node **tail)
{
    ...
    if (state->token_str)               //se il campo e' stato riempito
        create_real_token(state, head, tail); //crea finalmente un nuovo nodo con il token_str giusto e lo stato delle quote
    return (i); //ritorna la posizione del prossimo carattere utile
}