...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/05 12:58:33 by negambar          #+#    #+#             */
/*   Updated: 2024/10/05 12:59:02 by negambar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_token_to_list(t_token_node **head, \
		t_token_node **tail, char *token_str)
{
...
void	add_token_node(t_token_node **head, \
		t_token_node **tail, t_token_node *new_node)
{
	if (!*head) // Se la lista è vuota (head == NULL)
		*head = new_node; // Il nuovo nodo diventa la testa
	else
		(*tail)->next = new_node; // collega l'ultimo nodo al nuovo nodo
	*tail = new_node; // Aggiorna il puntatore tail per farlo puntare al
					// nuovo ultimo nodo
}
